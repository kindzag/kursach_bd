...
if(isset($_POST['find'])){
    ...
    if(empty($errors)){
        $sql = "SELECT employess.ide , employess.name, employess.surname, employess.patronymic,employess.birthday, departament.id_departament , departament.named AS departament, salary.sum_salary FROM employess JOIN departament ON employess.id_departament = departament.id_departament JOIN salary ON employess.ide = salary.ids WHERE employess.id_salary = '$id'";
...
<body>
<h3 class="reg-text">Поиск по ID зарплаты</h3>
